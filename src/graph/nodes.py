...
class AgenticRAGNodes:
    ...
    def _source_plan(self, query_type: str, requested_sources: list[str]) -> list[str]:
        ...
        requested = [s for s in requested_sources if s in allowed]
        if requested:
    ...
    def query_analyzer(self, state: RAGState) -> dict[str, Any]:
        ...
        if response:
            ...
            optimized_query = str(response.get("optimized_query", question)).strip() or question
    ...
    def generator(self, state: RAGState) -> dict[str, Any]:
        ...
        if llm_output:
            ...
            evidence = " ".join(docs[i]["content"][:130] for i in range(min(3, len(docs))))
